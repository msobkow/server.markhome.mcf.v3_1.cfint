...
public class CFIntJpaMimeTypeService {
	...
	public CFIntJpaMimeType create(CFIntJpaMimeType data) {
		...
		try {
			if(data.getPKey() != null && cfint31MimeTypeRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFIntJpaMimeType)(cfint31MimeTypeRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
	...
	public CFIntJpaMimeType update(CFIntJpaMimeType data) {
		...
		CFIntJpaMimeType existing = cfint31MimeTypeRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey()))
