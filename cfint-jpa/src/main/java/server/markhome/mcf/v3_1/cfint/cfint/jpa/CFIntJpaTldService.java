...
public class CFIntJpaTldService {
	...
	public CFIntJpaTld create(CFIntJpaTld data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfint31TldRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFIntJpaTld)(cfint31TldRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
	...
	public CFIntJpaTld update(CFIntJpaTld data) {
		...
		CFIntJpaTld existing = cfint31TldRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey()))
